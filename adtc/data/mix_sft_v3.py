#!/usr/bin/env python3
"""Assemble sft_mix_v3 from on-disk v2 pools (no new downloads / NLLB).
...
Example:
  python data/mix_sft_v3.py \\
    ...
    --am-stem data/train/sources/simonbutt_amharic_gsm8k_sft_v0.jsonl \\
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "data" / "train" / "sft_mix_v3.jsonl")
    ap.add_argument("--total", type=int, default=8000)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--en-ratio", type=float, default=0.25)
    ap.add_argument("--am-stem-ratio", type=float, default=0.45)
    ...
    ap.add_argument(
        "--counts-out",
        type=Path,
        default=ROOT / "docs" / "artifacts" / "sft_mix_v3_counts.json",
    )
    args = ap.parse_args()
