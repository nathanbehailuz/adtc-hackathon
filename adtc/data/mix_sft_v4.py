#!/usr/bin/env python3
"""Assemble sft_mix_v4: AfriqueLLM-first AM solve + fat tutoring; no simonbutt.

Target shares (default total 8000):
  AM STEM solve  40%
  AM tutoring    25%
  EN STEM        20%
  AM instruct    10%
...
Example:
  python data/mix_sft_v4.py \\
    ...
    --am-tutoring data/train/am_tutoring_sft_v4.jsonl \\
                  data/train/am_tutoring_derived_v4.jsonl \\
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "data" / "train" / "sft_mix_v4.jsonl")
    ap.add_argument("--total", type=int, default=8000)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--en-ratio", type=float, default=0.20)
    ap.add_argument("--am-stem-ratio", type=float, default=0.40)
    ap.add_argument("--am-tutoring-ratio", type=float, default=0.25)
    ap.add_argument("--am-instruct-ratio", type=float, default=0.10)
    ap.add_argument("--replay-ratio", type=float, default=0.05)
    ap.add_argument(
        "--counts-out",
        type=Path,
        default=ROOT / "docs" / "artifacts" / "sft_mix_v4_counts.json",
    )
    args = ap.parse_args()
    ...
    try:
        ...
        for p in args.en_stem:
            if not p.exists():
                raise FileNotFoundError(f"missing en-stem: {p}")
            chunk = read_jsonl(p)
            en_rows.extend(chunk)
            log.item_ok("en_stem", path=str(p), n_rows=len(chunk))

        missing_am = [p for p in args.am_stem if not p.exists()]
        if missing_am:
            raise FileNotFoundError(f"missing am-stem: {missing_am}")
        ...
        picked_replay = sample_pool(
            en_rows, max(0, n_replay + extra - max(0, extra // 2)), rng
        )
