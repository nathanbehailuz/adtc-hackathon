...
Training data are organized into CPT (optional) and SFT (default) stages. Named Hugging Face sources for Amharic are catalogued in `docs/DATASETS.md`. **Evaluation benchmarks (Iroko AfriMGSM/AfriMMLU/AfriXNLI, custom tutoring holdouts, GSM8K test) are never used for training.**

#### First-experiment recipe

Continued pretraining pools are prepared for conditional use; bilingual SFT is the default first adaptation:

\[
\underbrace{\text{FineWeb2-amh\_Ethi-100M + News + Wikipedia + AfriNLLB}}_{\text{CPT (if needed)}}
\rightarrow
\underbrace{\text{Walia + FineTome + Amharic STEM/GSM8K (+ EN STEM + verified MT)}}_{\text{SFT}}
\]

#### Native Target-Language Data (CPT)

Naturally produced Amharic text for grammar, morphology, idiom, and register. Primary sources:

* `MultilingualUnigramLM/FineWeb2-amh_Ethi-100M` (primary native web corpus for initial CPT);
* `dagn/expanded-amharic-news-dataset` (longer-form native news);
* `addisai/wikipedia-amharic` (encyclopedic / educational; STEM filtering preferred later);
* optionally `a3xrfgb/amharic-sentences-corpus` after aggressive filtering (social-media origin).

Full `HuggingFaceFW/fineweb-2` (`amh_Ethi`) is reserved for scale-up, not the first local download.

#### Parallel / educational alignment (CPT or auxiliary SFT)

* `AfriNLP/AfriNLLB-train` provides EN↔Amharic parallel pairs for continued alignment training, or can be transformed into explicit translation instructions for SFT.

#### Translated STEM Data

High-quality English educational and STEM material is translated into Amharic offline during dataset construction (not at inference). Sources include English GSM8K/SciQ tutoring templates (`build_en_stem_sft.py`) and machine translation (`build_translate_am.py`), plus Amharic math cold-start data from `peterlu02/afriquellm-coldstart-gsm8k-11lang` when language-filtered. Translated samples are reviewed by a fluent speaker for scientific terminology and register.

#### Reasoning-Preservation Data

English mathematics, science, structured reasoning, and optional code examples reduce catastrophic forgetting during multilingual adaptation (AfriqueLLM, 2026). English GSM8K **train** (not test) and SciQ train are retained; multilingual reasoning augmentation may use filtered `lightblue/reasoning-multilingual-R1-Llama-70B-train` after quality checks.

#### Amharic instruction SFT sources

* `EthioNLP/Amharic_Instruction_dataset` (Walia) — core Amharic instruction following;
* `addisai/FineTome-single-turn-dedup-amharic` — conversational / general instruct;
* supplementary after deduplication: `iocuydi/amharic-dolly-15k`, `CRLannister/Amharic`.

`YoseAli/amharic-llm-training-data` is **excluded wholesale** due to mixed provenance and contamination risk.

An initial CPT mixture (when CPT is triggered) is:
...
These proportions are an experimental starting configuration rather than a presumed optimum.
